...
def getModel(hyperParams = HyperParams(), performUserCheck=False):
  global WEIGHTS_PATH 
  WEIGHTS_PATH = os.path.join(WHEEL_CONTROLER_WEIGHTS_ROOT, "smallwheelmodel_" + hyperParams.getHyperParamSetName() + ".hdf5")
...
def loadModelWeights(model):
  input('Trying to load weights for wheel model from: ' + WEIGHTS_PATH)
  if os.path.isfile(WEIGHTS_PATH):
